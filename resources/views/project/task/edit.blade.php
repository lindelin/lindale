...
@section('content')
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <form action="{{ url('project/'.$project->id.'/task/task/'.$task->id) }}" method="post" role="form" enctype="multipart/form-data">
                {{ method_field('PATCH') }}
                {{ csrf_field() }}

                <h3><span class="glyphicon glyphicon-tasks"></span> {{ trans('task.edit-task') }}</h3>
                <hr>
                    ...
                    <div class="col-xs-12 col-sm-7 col-md-7 col-lg-7">
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            <div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
                                ...
                                <div>
                                    <input type="text" class="form-control" name="title" value="{{ old('title') ? old('title') : $task->title }}">
                            ...
                            <div class="form-group{{ $errors->has('start_at') ? ' has-error' : '' }}">
                                ...
                                <div>
                                    <div class='input-group date' id='datetimepicker1'>
                                        <input type='text' class="form-control" name="start_at" value="{{ old('start_at') ? old('start_at') : $task->start_at }}"/>
                            ...
                            <div class="form-group{{ $errors->has('end_at') ? ' has-error' : '' }}">
                                ...
                                <div>
                                    <div class='input-group date' id='datetimepicker2'>
                                        <input type='text' class="form-control" name="end_at" value="{{ old('end_at') ? old('end_at') : $task->end_at }}"/>
                                        <span class="input-group-addon">
                                            <span class="glyphicon glyphicon-calendar"></span>
                                        </span>
                                    </div>
                                ...
                                <div>
                                    <select class="selectpicker form-control" data-live-search="true" name="type_id">
                                        @foreach( $types as $type)
                                            <option value="{{ $type->id }}" @if(old('type_id') ? old('type_id') : $task->type_id == $type->id) selected @endif>
                                                {{ trans($type->name) }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group{{ $errors->has('user_id') ? ' has-error' : '' }}">
                                ...
                                <div>
                                    <select class="selectpicker form-control" data-live-search="true" name="user_id">
                                        <option value="0" @if(old('user_id') ? old('user_id') : $task->user_id == '' or 0) selected @endif>
                                            {{ trans('task.none') }}
                                        </option>

                                        <option value="{{ $project->ProjectLeader->id }}" @if(old('user_id') ? old('user_id') : $task->user_id == $project->ProjectLeader->id) selected @endif>
                                            {{ $project->ProjectLeader->name }}（{{ $project->ProjectLeader->email }}）
                                        </option>

                                        @if($project->SubLeader != null)
                                            <option value="{{ $project->SubLeader->id }}" @if(old('user_id') ? old('user_id') : $task->user_id == $project->SubLeader->id) selected @endif>
                                                {{ $project->SubLeader->name }}（{{ $project->SubLeader->email }}）
                                            </option>
                                        @endif
                                        @foreach( $users as $user)
                                            <option value="{{ $user->id }}" @if(old('user_id') ? old('user_id') : $task->user_id == $user->id) selected @endif>
                                ...
                                <div>
                                    <select class="selectpicker form-control" data-live-search="true" name="status_id">
                                        @foreach( $statuses as $status)
                                            <option value="{{ $status->id }}" @if(old('status_id') ? old('status_id') : $task->status_id == $status->id) selected @endif>
                                                {{ trans($status->name) }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    <select class="selectpicker form-control" name="priority_id">
                                        @foreach( $priorities as $priority)
                                            <option value="{{ $priority->id }}" @if(old('priority_id') ? old('priority_id') : $task->priority_id == $priority->id) selected @endif>
                                                {{ trans($priority->name) }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    <select class="selectpicker form-control" name="color_id">
                                        @foreach( config('color.common') as $id => $color)
                                            <option data-content="{{ Colorable::label($id, trans($color)) }}" value="{{ $id }}" @if(old('color_id') ? old('color_id') : $task->color_id === $id) selected @endif>
                                                {{ trans($color) }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group{{ $errors->has('cost') ? ' has-error' : '' }}">
                                ...
                                <div>
                                    <div class="input-group">
                                        <input type="text" class="form-control" name="cost" value="{{ old('cost') ? old('cost') : $task->cost }}">
                ...
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" align="center">
                        <a href="{{ url('project/'.$project->id.'/task/show/'.$task->id) }}" class="btn btn-success">{{ trans('task.cancel') }}</a>
                        <button type="submit" class="btn btn-warning">{{ trans('task.edit') }}</button>
                    </div>
                </div>
            </form>
        </div>
